    ...
    module DatabaseStatements
      ...
      private
        ...
        def should_pipeline_transactions?
          ...
          # 3. Raw connection is not dirty (leaked to userspace)
          has_unmaterialized = transaction_manager.has_unmaterialized_transactions?
          pipeline_supported = pipeline_supported?

          result = has_unmaterialized && pipeline_supported && !@raw_connection_dirty

          result
        end

        def should_pipeline_this_query?(sql, materialize_transactions, batch, pipeline_result_requested)
