  ...
  class PostgresqlPipelineTest < ActiveRecord::PostgreSQLTestCase
    ...
    def test_successful_intent_not_replayed_during_batch_recovery
      ...
      begin
        ...
        # Sync group 2: retryable query that self-destructs on the original
        # connection. Because it's in the pipeline after the sync boundary,
        # the first INSERT is committed before this query can kill the backend.
        self_destruct_once_sql = "SELECT" \
          " CASE WHEN pg_backend_pid() = #{initial_pid}" \
          " THEN pg_terminate_backend(pg_backend_pid()) ELSE true END," \
          " CASE WHEN pg_backend_pid() = #{initial_pid}" \
    ...
    def test_reconnectable_preserved_across_retry_in_ensure_result
      ...
      # the intent's retry budget must be preserved. attempt_retry marks
      # the budget's reconnect as consumed after the first reconnect
      # attempt; execute_intent's ||= guard prevents re-execution from
      # assigning a fresh budget with reconnectability restored.
      #
      ...
      begin
        ...
        # Verify initial state: the budget is reconnectable (from pipeline
        # initialization) with 2 retries remaining.
        assert_predicate intent2.retry_budget, :reconnectable?, "Should start reconnectable"
        assert_equal 2, intent2.retry_budget.retries_remaining

        # Intercept execute_intent to simulate what it does - the
        # `retry_budget ||=` assignment (the guard under test) - then
        # deliver a connection error. This exercises whether re-execution
        # preserves the budget whose reconnect was already consumed by
        # the retry loop.
        #
        # We cap at 5 invocations as a safety valve; without the ||=
        # guard this would infinite-loop (a fresh budget assigned each
        # time).
        ...
        conn.define_singleton_method(:execute_intent) do |i|
          if i.equal?(intent2)
            # Simulate the retry_budget assignment that execute_intent makes
            i.retry_budget ||= send(:build_retry_budget,
              allow_retry: i.allow_retry, reconnectable: send(:reconnect_can_restore_state?))
            reconnectable_after_init << i.retry_budget.reconnectable?

            if reconnectable_after_init.size >= 5
              # Safety: break infinite loop by making intent non-retriable
              i.retry_budget.instance_variable_set(:@retries_remaining, 0)
            end
        ...
        # With the ||= guard: execute_intent is called once, the budget's
        # reconnect stays consumed, attempt_retry declines, loop breaks.
        # Without it: reconnectability resets to true each time, causing
        # repeated reconnect attempts.
        assert_equal [false], reconnectable_after_init,
          "budget reconnectability should be preserved as false across re-execution; " \
    ...
    private
      ...
      def close_client_socket(conn)
        raw_conn = conn.instance_variable_get(:@raw_connection)
        raw_conn.socket_io.shutdown(Socket::SHUT_RDWR)
      end
  end
